...
class EnergyVAD:
    ...
    def __init__(self, on_packet: OnPacket, *, threshold: float = 0.0035,
                 speech_frames: int = 2, silence_frames: int = 15) -> None:
    ...
    def probability(self, pcm: bytes) -> float:
        ...
        threshold = max(self._base_threshold, self._floor * 1.8)
        if level <= threshold:
            return 0.0
